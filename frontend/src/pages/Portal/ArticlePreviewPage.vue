    <template>
      ...
      <div v-else-if="article" class="article-page">
        ...
        <div class="entity-shell article-shell article-grid">
          <div class="article-cover" :class="{ 'article-cover--image': isCoverImage }" :style="articleCoverStyle">
                ...
                <v-card class="dialog-card card card-rounded-xl" rounded="xl">
                  <div class="feedback-modal">
                    ...
                    <div class="feedback-modal-footer">
                      <button class="feedback-cancel" type="button" @click="feedbackDialog = false">Annuleren</button>
                      <button class="feedback-submit" type="button" @click="submitFeedback"
          ...
          <main class="article-content">
            <article class="article-card card card-elevated card-rounded-2xl">
              ...
              <div class="article-body" id="content" v-html="article.content" @mouseover="showLinkPreview"
                @mousemove="moveLinkPreview" @mouseout="hideLinkPreview" @focusin="showLinkPreview"
                @focusout="hideLinkPreview" />

              <div v-if="linkPreview.visible" class="link-preview-tooltip" :style="linkPreviewStyle">
                <img v-if="linkPreview.thumbnail" :src="linkPreview.thumbnail" :alt="linkPreview.title"
              ...
              <div v-if="articleAttachments.length" id="attachments" class="resource-grid">
                <button v-for="attachment in imageAttachments" :key="attachment.id" type="button"
                  class="resource-card resource-card-image" @click="openImagePreview(attachment)">
                  <img :src="getAttachmentUrl(attachment)" :alt="getAttachmentName(attachment)"
                ...
                <a v-for="attachment in downloadableAttachments" :key="attachment.id"
                  class="resource-card resource-card-download" :href="getAttachmentUrl(attachment)"
                  ...
                  <span class="resource-copy">
                    <span class="resource-title">{{ getAttachmentName(attachment) }}</span>
                    <span class="resource-subtitle">{{ isPdfAttachment(attachment) ? 'PDF downloaden' : 'Downloaden' }}
...
const articleCoverStyle = computed(() => {
  ...
  if (coverAttachment) {
    ...
    if (coverUrl) {
      return {
        backgroundColor: DEFAULT_COVER_COLOUR,
  ...
  return {
    backgroundColor: normalizeColourInput(articleCoverValue.value) || DEFAULT_COVER_COLOUR,
